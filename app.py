...
with tab_hr:
    ...
    iqama = st.number_input("رسوم إقامات", value=600)
    ...
    admin_cost = st.number_input("مصاريف إدارية", value=24000)
...
with tab_comp:
    ...
    st.info("حساب التجهيز والهالك والمستهلكات")

    cc1, cc2 = st.columns(2)

    with cc1:
        st.subheader("إعدادات الطلبية")
        job_colors = st.number_input("ألوان", value=8)
        avg_mat_cost = st.number_input("تكلفة مواد الطن", value=9000)
        meters_per_ton = st.number_input("أمتار الطن", value=20000)
    ...
    with cc2:
        ...
        f_waste = st.number_input("هالك فلكسو", value=50)
        r_waste = st.number_input("هالك روتو", value=250)

    st.markdown("---")
    st.subheader("المستهلكات")
    
    c_cons1, c_cons2 = st.columns(2)
    with c_cons1:
        anilox_p = st.number_input("سعر أنيلوكس", value=15000)
        anilox_l = st.number_input("عمر أنيلوكس", value=200000000)
    ...
    for t in job_sizes:
        f_c = avg_mat_cost + (tot_f_fixed / t) + (tot_f_cons * meters_per_ton)
        r_c = avg_mat_cost + (tot_r_fixed / t) + (tot_r_cons * meters_per_ton)
...
with tab_mix:
    ...
    tot_vol = st.number_input("إجمالي الطلب السنوي", value=1200)

    crm1, crm2 = st.columns(2)

    with crm1:
        st.subheader("نسب الروتو")
        r_data = [
            {"الهيكل": "طبقة", "النسبة": 30.0, "السعر": 13.0},
            {"الهيكل": "طبقتين", "النسبة": 50.0, "السعر": 13.5},
            {"الهيكل": "3 طبقات", "النسبة": 20.0, "السعر": 15.0},
        ]
        df_r_mix = st.data_editor(pd.DataFrame(r_data), use_container_width=True, key="r_mix")
        
        df_r_mix["الكمية"] = tot_vol * (df_r_mix["النسبة"] / 100.0)
        df_r_mix["الإيراد"] = df_r_mix["الكمية"] * df_r_mix["السعر"] * 1000
        
        r_tot_rev = df_r_mix["الإيراد"].sum()
        r_avg_p = r_tot_rev / (tot_vol * 1000) if tot_vol > 0 else 0
            
        st.metric("إيرادات الروتو", fmt(r_tot_rev))
        st.metric("متوسط سعر الروتو", fmt1(r_avg_p))

    with crm2:
        st.subheader("نسب الفلكسو")
        f_data = [
            {"الهيكل": "طبقة", "النسبة": 60.0, "السعر": 12.0},
            {"الهيكل": "طبقتين", "النسبة": 30.0, "السعر": 13.0},
            {"الهيكل": "3 طبقات", "النسبة": 10.0, "السعر": 15.0},
        ]
        df_f_mix = st.data_editor(pd.DataFrame(f_data), use_container_width=True, key="f_mix")

        df_f_mix["الكمية"] = tot_vol * (df_f_mix["النسبة"] / 100.0)
        df_f_mix["الإيراد"] = df_f_mix["الكمية"] * df_f_mix["السعر"] * 1000
        
        f_tot_rev = df_f_mix["الإيراد"].sum()
        f_avg_p = f_tot_rev / (tot_vol * 1000) if tot_vol > 0 else 0
            
        st.metric("إيرادات الفلكسو", fmt(f_tot_rev))
        st.metric("متوسط سعر الفلكسو", fmt1(f_avg_p))

    st.markdown("---")
    st.subheader("مقارنة الإيرادات")
    ...
    with cv1:
        fig_v = px.bar(df_cmb, x="التقنية", y="الكمية", color="الهيكل", barmode="group")
        st.plotly_chart(fig_v, use_container_width=True)
        
    with cv2:
        fig_r = px.bar(df_cmb, x="التقنية", y="الإيراد", color="الهيكل", barmode="group")

...
with tab_lamination:
    ...
    with col_l1:
        num_layers = st.selectbox("عدد الطبقات | Number of Layers", [1, 2, 3, 4], format_func=lambda x: "1 (بدون لامنيشن | No Lamination)" if x == 1 else str(x))
        passes = max(0, num_layers - 1)
        
        if passes > 0:
            adhesive_gsm = st.number_input("غراء للتمريرة | Adhesive (g/m2/pass)", value=1.8)
    ...
    with col_l2:
        ...
        if num_layers > 1:
            for i in range(2, num_layers + 1):
                ...
                layer_mat_name = col_mat.selectbox(f"مادة الطبقة {i} | Layer {i} Material", list(materials_db.keys()), key=f"mat_{i}")
                layer_thk = col_thk.number_input(f"سماكة | Thickness (Microns)", value=20, key=f"thk_{i}")
    ...
    st.markdown("---")
    st.subheader("⚙️ طاقة اللامنيشن | Lamination Capacity")
    col_cap1, col_cap2 = st.columns(2)
    with col_cap1:
        lam_machine_speed = st.slider("سرعة اللامنيشن | Lam Speed (m/min)", 100, 500, 350)
    ...
    with col_cap2:
        ...
        st.write(f"🔄 مطلوب للامنيشن | Required Lam Meters: **{total_lam_run_meters:,.0f} m**")
        st.write(f"🏭 طاقة الماكينة | Machine Max Capacity: **{lam_max_capacity_meters:,.0f} m**")

        if passes == 0:
            st.success("✅ طباعة فقط (بدون لامنيشن) | Surface Printing Only (No Lamination)")
        elif utilization <= 100:
            st.success(f"✅ استهلاك الماكينة | Utilization: **{utilization:.1f}%**")
        else:
            st.error(f"⚠️ تحذير اختناق | Bottleneck Alert: **{utilization:.1f}%**!")

    st.markdown("---")
    col_out1, col_out2, col_out3 = st.columns(3)
    col_out1.metric("إجمالي تكلفة المواد | Total Materials Cost", f"{total_raw_materials_cost:,.0f} SAR")
    col_out2.metric("استهلاك الغراء | Adhesive Consumed", f"{adhesive_consumed_kg:,.0f} Kg")
    col_out3.metric("الوزن النهائي | Final Sale Weight", f"{final_production_tons:,.1f} Ton")

# ==========================================
# TAB 4: الماكينات والأصول (Machinery)
# ==========================================
with tab_machines:
    st.header("إدارة الأصول والطاقة | Assets & Utilities")
    col_elec1, col_elec2 = st.columns(2)
    electricity_rate = col_elec1.number_input("سعر الكيلوواط | Electricity Rate (SAR/kWh)", value=0.18)
    working_hours_per_month = col_elec2.number_input("ساعات التشغيل | Monthly Hours", value=624)

    default_machines = pd.DataFrame([
        {"Machine / الماكينة": "CI Flexo | فلكسو", "Cost_SAR": 8000000, "Life_Years": 15, "Power_kW": 150},
        {"Machine / الماكينة": "Solventless Lam | لامنيشن", "Cost_SAR": 1200000, "Life_Years": 15, "Power_kW": 125},
        {"Machine / الماكينة": "PE Extruder | إكسترودر", "Cost_SAR": 5000000, "Life_Years": 15, "Power_kW": 250},
        {"Machine / الماكينة": "Slitter | قطاعة", "Cost_SAR": 800000, "Life_Years": 15, "Power_kW": 40},
        {"Machine / الماكينة": "Bag Making | أكياس", "Cost_SAR": 620000, "Life_Years": 10, "Power_kW": 50},
        {"Machine / الماكينة": "Chiller & Comp | مبرد", "Cost_SAR": 600000, "Life_Years": 15, "Power_kW": 90},
        {"Machine / الماكينة": "Building | المبنى", "Cost_SAR": 4000000, "Life_Years": 25, "Power_kW": 0},
    ])

    edited_machines = st.data_editor(default_machines, num_rows="dynamic", use_container_width=True)
    edited_machines["Depreciation_SAR"] = edited_machines["Cost_SAR"] / (edited_machines["Life_Years"] * 12)
    edited_machines["Power_SAR"] = edited_machines["Power_kW"] * working_hours_per_month * 0.85 * electricity_rate

    total_capex = edited_machines["Cost_SAR"].sum()
    total_monthly_depreciation = edited_machines["Depreciation_SAR"].sum()
    total_monthly_power = edited_machines["Power_SAR"].sum()

# ==========================================
# TAB 5: الموارد البشرية والإدارة (HR & Admin)
# ==========================================
with tab_hr_admin:
    st.header("الموارد البشرية والإدارة | HR, Admin & Logistics")

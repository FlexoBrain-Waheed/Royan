...
with tab_fin:
    ...
    total_cost = (total_raw_cost + ink_cost_monthly + solvent_cost_monthly + 
                  adhesive_cost + tot_power + tot_deprec + grand_hr_admin)
    revenue = final_tons * selling_price
    profit = revenue - total_cost
    cpt = total_cost / final_tons if final_tons > 0 else 0
    ...
    chart_data = {
        ...
        "Cost": [total_raw_cost, (ink_cost_monthly + solvent_cost_monthly + adhesive_cost), 
                 (tot_power + tot_deprec), tot_hr_cost, (truck_cost + admin_cost)]
    }
...
with tab_comp:
    ...
    sub_tabs = st.tabs(["1. المقارنة المالية", "2. الكفاءة التشغيلية", "3. التخزين"])

    with sub_tabs[0]:
        st.subheader("تحليل التكاليف للطلبية")
        cc1, cc2 = st.columns(2)
        with cc1:
            job_colors = st.number_input("الالوان", value=8)
        ...
        with c_mac2:
            r_mac_price = st.number_input("سعر روتو + غلاية", value=11500000)
        ...
        f_mac_per_ton = (f_mac_price / 180) + (f_kw * 624 * 0.18) / 300
        r_mac_per_ton = (r_mac_price / 180) + (r_kw * 624 * 0.18) / 300
    ...
    with sub_tabs[1]:
        st.subheader("الوقت الضائع (Opportunity Cost)")
        co1, co2 = st.columns(2)
        with co1:
            r_setup_min = st.number_input("تجهيز روتو (دقيقة)", value=150)
            f_setup_min = st.number_input("تجهيز فلكسو (دقيقة)", value=45)
        with co2:
            monthly_jobs = st.number_input("طلبيات الشهر", value=60)
            profit_per_hour = st.number_input("ربح الساعة", value=1500)

        saved_hrs = ((r_setup_min * monthly_jobs) - (f_setup_min * monthly_jobs)) / 60.0
        extra_profit = saved_hrs * profit_per_hour
        st.metric("الربح الاضافي لتوفير الوقت", fmt(extra_profit) + " ريال/شهر")

    with sub_tabs[2]:
        st.subheader("تكلفة التخزين")
        cs1, cs2 = st.columns(2)
        with cs1:
            total_designs = st.number_input("عدد التصاميم", value=500)
            colors_per_d = st.number_input("الوان التصميم", value=8)
        with cs2:
            rent_sqm = st.number_input("ايجار المتر المربع", value=300)
            
        total_units = total_designs * colors_per_d
        r_rent_cost = (total_units / 100) * 5 * rent_sqm
        f_rent_cost = (total_units / 100) * 1 * rent_sqm
        
        cr1, cr2 = st.columns(2)
        cr1.metric("تكلفة التخزين للروتو", fmt(r_rent_cost) + " ريال")
        cr2.metric("تكلفة التخزين للفلكسو", fmt(f_rent_cost) + " ريال")
...
with tab_mix:
    ...
    with crm1:
        ...
        st.metric("متوسط السعر", fmt1(r_avg_p))
    ...
    with crm2:
        ...
        st.metric("متوسط السعر", fmt1(f_avg_p))
